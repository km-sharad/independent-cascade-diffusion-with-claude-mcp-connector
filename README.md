...
<br/>
The methods decorated with <code>@mcp.tool()</code> annotation in MCP servers is available to Claude to call in response to a queries. Some representative methods are <code>analyze_node_susceptibility()</code>, <code>analyze_seed_impact</code> and <code>compare_method_reach()</code>. Claude Desktop uses these tools to retrive network diffusion related information from the JSON file to answer user questions.
</br>
<i>MCP Code</i>
</br>
