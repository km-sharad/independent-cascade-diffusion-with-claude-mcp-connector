...
<br/>
The methods decorated with `@mcp.tool()` annotation in MCP servers is available to Claude to call in response to a queries. Some representative methods are `analyze_node_susceptibility()`, `analyze_seed_impact` and `compare_method_reach()`. Claude Desktop uses these tools to retrive network diffusion related information from the JSON file to answer user questions.
</br>
<i>MCP Code</i>
</br>
