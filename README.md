# Graph Data Mined using Independent Cascade Diffusion Integrated with Claude MCP Connector
...
<li>If it takes $50 to activate odd number nodes and $100 to activate even number nodes (a node property can be substituted for odd and even number nodes), which method is the most economical to reach at least 2000 people?</li>
</ul>
...
The statistics generated by the diffusion algorithm is saved in a JSON file and is available in the codebase here: <a href="https://raw.githubusercontent.com/km-sharad/independent-cascade-diffusion-with-claude-mcp-connector/refs/heads/main/network-diffusion/data/diffusion_stats/network_centrality_results_with_logs.json">network-diffusion/data/diffusion_stats/network_centrality_results_with_logs.json</a>. Next, I explain how the statistics available in this file is used by the MCP connector.

<h3>Integration with Claude Desktop using Model Context Protocol (MCP)</h3>
MCP (Model Context Protocol) is an open-source protocol created by Anthropic that enables AI assistants like Claude to connect to external data sources and tools. I created MCP servers that use the statistics available in this <a href="https://raw.githubusercontent.com/km-sharad/independent-cascade-diffusion-with-claude-mcp-connector/refs/heads/main/network-diffusion/data/diffusion_stats/network_centrality_results_with_logs.json">JSON file</a> about the diffusion in the network to obtain context knowledge for answering user questions about static and dynamic network behavior . 
<br/>
The methods decorated with <code>@mcp.tool()</code> annotation in MCP servers is available to Claude to call in response to a query. Some representative methods are <code>analyze_node_susceptibility()</code>, <code>analyze_seed_impact</code> and <code>compare_method_reach()</code>. Claude Desktop uses these tools to retrive network diffusion related information from the JSON file to answer user questions.
</br>
<i>MCP Code</i>
</br>
